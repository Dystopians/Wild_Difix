...
from dataset_json import PairedDatasetJSON
...
def main(args):
    ...
    dataset_train = PairedDatasetJSON(dataset_path=args.dataset_path, split="train", tokenizer=net_difix.tokenizer)
    dl_train = torch.utils.data.DataLoader(dataset_train, batch_size=args.train_batch_size, shuffle=True, num_workers=args.dataloader_num_workers)
    dataset_val = PairedDatasetJSON(dataset_path=args.dataset_path, split="test", tokenizer=net_difix.tokenizer)
    ...
    for epoch in range(0, args.num_training_epochs):
        for step, batch in enumerate(dl_train):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                # forward pass
                x_tgt_pred = net_difix(x_src, prompt_tokens=batch.get("input_ids"))       
                ...
                if accelerator.is_main_process:
                    ...
                    if args.eval_freq > 0 and global_step % args.eval_freq == 1:
                        ...
                        for step, batch_val in enumerate(dl_val):
                            ...
                            with torch.no_grad():
                                x_tgt_pred = accelerator.unwrap_model(net_difix)(x_src, prompt_tokens=batch_val.get("input_ids"))
